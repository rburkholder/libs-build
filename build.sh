...
function build_boost {
  ...
  pushd ${boost_dir}
  sudo ./b2 --layout=versioned toolset=gcc cxxflags=-std=gnu++17 variant=release link=static,shared threading=multi address-model=64 -j2 install
...
wxwidgets_ver="${wxwidgets_ver_major=}.2.1"
...
case "$1" in
  ...
  tradeframe)
    base
    sudo apt-get -y install libcurl4-openssl-dev libintelrdfpmath-dev gnulib libcrypto++-dev
    boost
    wx
    hdf5
    chartdir
